...
class VerticalShapes(CompoundShapes):
    ...
    def _get_postscript(self, center): #perheps this could be more DRY if it was passed a flag for h||w in compound class?
        shape_exports = []
        current_y = center.y - self._get_height() / 2
        for shape in self._shapes:
            half_shape_height = shape._get_height() / 2
            current_y += half_shape_height
            shape_exports.append(shape._get_postscript(Point(center.x, current_y)))
            current_y += half_shape_height
