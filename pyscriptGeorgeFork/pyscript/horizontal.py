...
class HorizontalShapes(CompoundShapes):
    ...
    def _get_shapes_postscript(self, center): #perheps this could be more DRY if it was passed a flag for h||w in compound class?
        shape_exports = []
        current_x = center.x - self._get_width() / 2
        for shape in self._shapes:
            half_shape_width = shape._get_width() / 2
            current_x += half_shape_width
            shape_exports.append(shape._get_postscript(Point(current_x, center.y)))
            current_x += half_shape_width
